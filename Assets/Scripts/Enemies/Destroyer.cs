...
namespace Enemies
{
    public class Destroyer: Enemy
    {
        protected override void MakeMove()
        {
            ...
            var damage1 = weapons.autoCannon.CalculateDamage(Player) * ActionPoints;
            MoveToPlayerOnX();
            if (damage1 == 0) return; // if can't deal damage now, stay in the new position
            var damage2 = weapons.autoCannon.CalculateDamage(Player) * (ActionPoints - 1);
            if (damage2 > damage1) return; // if can increase damage, stay in the new position
            if (Math.Abs(deltaX) == 1)
            {
                var damage3 = weapons.grandCannon.CalculateDamage(Player) * (ActionPoints - 1);
